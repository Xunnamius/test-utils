[**@-xun/common-dummies**](../README.md)

***

[@-xun/common-dummies](../README.md) / repositories

# repositories

## Type Aliases

- [PackageMapDatum](type-aliases/PackageMapDatum.md)
- [PackageMapEntry](type-aliases/PackageMapEntry.md)
- [Repository](type-aliases/Repository.md)
- [RepositoryName](type-aliases/RepositoryName.md)

## Variables

- [repositories](variables/repositories.md)

## Functions

- [dummyToProjectMetadata](functions/dummyToProjectMetadata.md)
- [patchJsonObjectReaders](functions/patchJsonObjectReaders.md)
